...
# Clean up custExists variable - Mainly For re-testing script in ISE
$custExists = ''
$skipToInstall = ''
...
If ($testmode -eq "false") {
    ...
    # Create download job
    Start-Job -Name downloadInstaller -ScriptBlock { $installerDownload = Invoke-WebRequest -Uri "http://download.bitdefender.com/business/misc/kb1695/eps_installer_signed.zip" -OutFile "$env:temp/eps_installer_signed.zip" -PassThru }

    # Wait until the download has completed (accounts for slow internet connections)
    Wait-Job -Name downloadInstaller

    # Check the download has completed successfully, fail and exit if not.
    If ($installerDownload.StatusDescription -eq "OK"){
        write-host "eps installer downloaded"
    }
    Else {
        Write-Host "Download failed. Aborting install."
        exit
    }

    # Attempt to expand archive. If it fails then stop script.
    $expandArchive = Expand-Archive "$env:temp/eps_installer_signed.zip" -DestinationPath $env:temp/eps_installer_signed/ -Force -ErrorAction Stop
    ...
    write-host "Executing msi installer with SiteCode"
